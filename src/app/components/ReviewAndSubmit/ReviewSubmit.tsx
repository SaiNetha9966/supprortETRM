...
export const ReviewSubmit: React.FC<ReviewSubmitProps> = ({
  ...
  const getToolNameFromSlug = (slug: string) => {
    const match = toolCatalog.find((tool: any) =>
  ...
  const existingToolsFromMetadata = (existingProjectMetadata?.result?.existingtools ?? []).map((slug: string) => ({
  ...
  const newToolsFromExistingFlow = (existingToolFormData?.selectedTools ?? []).filter(
    (tool: any) => !existingToolSlugs.has(toSlug(tool?.ToolName ?? tool?.Tool_name ?? tool?.name ?? ''))
  );

  const newToolSlugs = new Set(
    newToolsFromExistingFlow.map((tool: any) => toSlug(tool?.ToolName ?? tool?.Tool_name ?? tool?.name ?? ''))
  );
  ...
  const existingAccessList = Object.entries(
    normalizeNameValueMap(existingRecord?.namevalue ?? existingRecord?.nameValuePairs ?? existingRecord?.nameValue ?? {})
  ).map(([name, tools]) => ({
    name,
          ...
          <div>
            ...
            <div className={styles.detailsGrid}>
              ...
              <div className={styles.detailItem}>
                ...
                <div className={styles.value}>{isExistingProject ? 'Onboarding, Existing Project' : 'Onboarding, New Project'}</div>
              </div>
              <div className={styles.detailItem}>
        ...
        <div className={styles.card} style={{ gridColumn: '1 / -1' }}>
          ...
          <div className={styles.detailsGrid}>
            <div className={styles.detailItem}>
              ...
              <div className={styles.value}>{isExistingProject ? selectedProjectKey : formData.ertmProjectId}</div>
            </div>
            <div className={styles.detailItem}>
              <div className={styles.label}>SAP Project ID</div>
              <div className={styles.value}>{isExistingProject ? (existingRecord?.sap_project_id ?? existingRecord?.sapProjectId) : formData.sapProjectId}</div>
            </div>
            <div className={styles.detailItem}>
              <div className={styles.label}>Project Code Name</div>
              <div className={styles.value}>{isExistingProject ? (existingRecord?.project_code_name ?? existingRecord?.codename ?? existingRecord?.projectCodeName) : formData.projectCodeName}</div>
            </div>

            <div className={styles.detailItem}>
              <div className={styles.label}>Project Type</div>
              <div className={styles.value}>{isExistingProject ? (existingRecord?.project_type ?? existingRecord?.projectType) : formData.projectType}</div>
            </div>
            <div className={styles.detailItem}>
              <div className={styles.label}>Estimated Start Date</div>
              <div className={styles.value}>{formatDate(isExistingProject ? (existingRecord?.estimated_start_date ?? existingRecord?.estimatedStartDate) : formData.estimatedStartDate)}</div>
            </div>
            <div className={styles.detailItem}>
              <div className={styles.label}>Estimated End Date</div>
              <div className={styles.value}>{formatDate(isExistingProject ? (existingRecord?.estimated_end_date ?? existingRecord?.estimatedEndDate) : formData.estimatedEndDate)}</div>
            </div>

            <div className={styles.detailItem}>
              <div className={styles.label}>Is Personal or Protected Data Involved?</div>
              <div className={styles.value}>{isExistingProject ? (existingRecord?.are_you_planning_to_use_any_personal_or_protected_data ?? existingRecord?.personalOrprotectedData) : formData.personalOrprotectedData}</div>
            </div>
            <div className={styles.detailItem}>
              <div className={styles.label}>Describe your project and its goals.</div>
              <div className={styles.value}>{isExistingProject ? (existingRecord?.please_describe ?? existingRecord?.description) : formData.description}</div>
            </div>
          </div>
        </div>
        ...
        <div className={styles.card} style={{ gridColumn: '1 / -1' }}>
          <div className={styles.cardHeader}>
            <div className={styles.cardTitle}>
              ...
              {isExistingProject && (
                <span>
                  {' '}({existingToolsFromMetadata.length} existing tools · {newToolsFromExistingFlow.length} new tool{newToolsFromExistingFlow.length === 1 ? '' : 's'} added)
                </span>
              )}
            </div>
            <EditButton />
          ...
          <div className={styles.toolsGrid}>
            {toolSpecsForDisplay.map((tool: any, index: number) => (
              <div key={index} className={styles.toolCard}>
                ...
                <div className={styles.value}>{isYesValue(tool.trustExternalDomain) ? "Yes" : "No"}</div>
        ...
        <div className={styles.card} style={{ gridColumn: '1 / -1' }}>
          ...
          <div className={styles.approverGrid}>
              <div className={styles.approverItem}>
                ...
                <div className={styles.approverName}>{findNameByEmail(formData.primaryPmdPartner, usersList)}</div>
              </div>
               <div className={styles.approverItem}>
                <div className={styles.approverTitle}>Secondary PMD/Partner</div>
                <div className={styles.approverName}>{findNameByEmail(formData.secondoryPmdPartner,usersList)}</div>
              </div>
               <div className={styles.approverItem}>
                <div className={styles.approverTitle}>Information Owner</div>
                <div className={styles.approverName}>{findNameByEmail(formData.informationOwner,usersList)}</div>
              </div>
               <div className={styles.approverItem}>
                <div className={styles.approverTitle}>Delegate Information Owner</div>
                <div className={styles.approverName}>{findNameByEmail(formData.delegateIformationOwner,usersList)}</div>
              </div>
               <div className={styles.approverItem}>
                <div className={styles.approverTitle}>Project Manager</div>
                <div className={styles.approverName}>{findNameByEmail(formData.projectManager,usersList)}</div>
              </div>
               <div className={styles.approverItem}>
                <div className={styles.approverTitle}>Approvers</div>
                <div className={styles.approverName}>{findNameByEmail(formData.approvers,usersList)}</div>
              </div>
          </div>
        </div>
                ...
                <div className={styles.accessMain} onClick={() => toggleAccess(idx)}>
                  <div className={styles.accessInfo}>
                    <div className={styles.accessName}>
                      {isExistingProject ? (a.name || '') : findNameByEmail(a?.email?.trim(), usersList)}
                    </div>
                    <div className={styles.accessEmail}>{a.email || ''}</div>
                  </div>
                  <div className={styles.accessRight}>
                    <div className={styles.rolePill}>Limited Access {a?.toolAccess?.length || 0}</div>
                    <button
