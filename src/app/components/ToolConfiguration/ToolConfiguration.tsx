...
export default function ToolConfiguration({ data, formData, setFormData, handleChange }: ToolConfigurationProps) {
...
const toggleTool = (toolId: string) => {
  setFormData((prev: any) => {
    ...
    if (isToolSelected) {
      ...
      const toolToAdd = allTools.find((tool: any) => tool.id === toolId || tool.ToolId === toolId);
      if (toolToAdd) {
        return {
          ...prev,
          selectedTools: [
...
<div className="rounded-lg p-4 sm:p-5">
  <div className="flex flex-col gap-4 sm:gap-5">
    <div className="flex items-center justify-between gap-4">
      ...
      <button onClick={() => setShowAddToolsModal(true)} className="flex items-center gap-1 px-4 py-2 border border-[#498E2B] text-[#498E2B] hover:bg-[#f1f6f0] transition-colors text-[12px] sm:text-[13px] md:text-[14px] font-medium" style={{ width: "152px", height: "32px" }}>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5 md:gap-6">
      {
          uniqueTools.map((tool: Tool) => {
            const isSelected = formData.selectedTools.some((selected: any) => selected.ToolId === tool.ToolId);
            return <ToolCard
              key={tool.ToolId}
              name={tool.ToolName}
          ...
          <div className="flex flex-col gap-5 sm:gap-6 md:gap-8">
            <div className="flex flex-col gap-3">
              ...
              <p className="text-[13px] sm:text-[14px] md:text-[15px] font-medium text-[#727272]">
                Based on your project details, the tools below require configuration.Completely only the fields marked as required.
              </p>
