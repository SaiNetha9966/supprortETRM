...
export const Dropdown: React.FC<DropdownProps> = ({ label, required, hasInfo, onChange, selected, setSelected, options }) => {
    ...
    return (
        <div className="flex flex-col gap-1.5 w-full relative font-['Roboto',sans-serif]">
            ...
            >
                ...
                {
                    !open ? <ChevronDown className="w-4 h-4 text-[#4a4a4a]" /> : <ChevronDown className="w-4 h-4 text-[#4a4a4a] rotate-180" />
                }
            </div>
            ...
            {open && (
                <div className="absolute z-10 mt-17 w-full border border-[#ccc] bg-white rounded shadow-lg p-2">
                    ...
                    {filteredGroups.map((group) => (
                        <div key={group.label} className="mb-2">
                            ...
                            {group.options.map((opt) => (
                                <div
                                    key={opt.value}
                                    className={`text-sm px-2 py-1 rounded cursor-pointer ${selected.includes(opt.value) ? "bg-green-100 text-green-700" : "hover:bg-gray-100"
                                        }`}
                    ...
                    <div className="flex justify-between mt-2 px-1">
                        <Buttons variant="secondary" onClick={() => { setSelected([]); onChange?.([]); setOpen(false); }}>Clear</Buttons>
                        <Buttons variant="primary" onClick={() => { onChange?.(selected); setOpen(false); }} >Add</Buttons>
